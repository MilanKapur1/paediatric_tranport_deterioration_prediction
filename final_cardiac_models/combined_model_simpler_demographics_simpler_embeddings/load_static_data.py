...
def load_static_data (patient_list):    
    ...
    df = df.drop(columns=[
        ...
        'ethnicity_name','cats_interv','min_session_time'
    ])
    ...
    #######################################################################################################################
    ###onehot encode diagnosis into groups:

        
    df['diagnosis_group'] = df['diagnosis_group'].astype(str)
    df['primary_diagnosis'] = df['primary_diagnosis'].astype(str).str.lower()

    # 2. Create masks for your custom logic
    #    Respiratory if primary_diagnosis contains "bronch"
    resp_mask = df['primary_diagnosis'].str.contains('bronch|brochio|airwa|epiglott|lrti|smoke inhalation|pertussis|subglottic|choki|respirat|stridor|apnoea|pneu|trach|aspir|asthma|empyema|pulmon|wheeze|lung|pleura|croup', na=False)
    #    Cardiovascular if primary_diagnosis contains "card", "coarc", or "heart"
    card_mask = df['primary_diagnosis'].str.contains('card|coarc|svt|heart|ecmo|ebstein|arryth|ventric|aort|vasc|tetralogy|transposit|rhythm|arte|mitra|atria|atrio|shock', na=False)
    neuro_mask = df['primary_diagnosis'].str.contains('neuro|seizure|epile|crani|encephalomyelitis|dystonia|cerebr|myasth|hydrocephalus|myelo|convul|guill|encephalopathy|brain|dura', na=False)
    infection_mask = df['primary_diagnosis'].str.contains('infect|systemic inflammatory|meningitis|encephalitis|cellulitis|sepsis|conscio|spina|galen|meningococcaemia|abscess', na=False)& ~df['primary_diagnosis'].str.contains('respirat', na=False)
    gastro_mask = df['primary_diagnosis'].str.contains('abdom|bowel|hernia|duoden|chylo|omphalo|oesopha|appendi|pylor|bili|intesti|exompha|colitis|hepati|gastr', na=False)
    metabolic_mask = df['primary_diagnosis'].str.contains('diabe|metaboli|glycae|ammonae', na=False)
    #renal_mask = df['primary_diagnosis'].str.contains('urae|urea|natrae|kalaem|nephro|renal|kidney', na=False)
    trauma_mask = df['primary_diagnosis'].str.contains('burn|injury|traum|submersion', na=False)
    #haem_mask = df['primary_diagnosis'].str.contains('sickle|haemophag', na=False)
    #onc_mask = df['primary_diagnosis'].str.contains('leuk|neutrop|teratoma', na=False)


    # 2. Overwrite the diagnosis_group based on these masks
    df.loc[resp_mask,      'diagnosis_group'] = 'DiagnosisRespiratory'
    df.loc[card_mask,      'diagnosis_group'] = 'DiagnosisCardiovascular'
    df.loc[neuro_mask,     'diagnosis_group'] = 'DiagnosisNeurological'
    df.loc[infection_mask, 'diagnosis_group'] = 'DiagnosisInfection'
    df.loc[gastro_mask,    'diagnosis_group'] = 'DiagnosisGastrointestinal'
    df.loc[metabolic_mask, 'diagnosis_group'] = 'DiagnosisMetabolic'
    #df.loc[renal_mask,     'diagnosis_group'] = 'Renal'
    df.loc[trauma_mask,    'diagnosis_group'] = 'DiagnosisTrauma'
    #df.loc[haem_mask,      'diagnosis_group'] = 'Haematological'
    #df.loc[onc_mask,       'diagnosis_group'] = 'Oncological'
    

    # 3. Any rows *not* matching any mask => "Other"
    # Create a combined mask of all categories above, then invert it.
    all_masks = (resp_mask | card_mask | neuro_mask | infection_mask | gastro_mask |
                 metabolic_mask | trauma_mask)
    df.loc[~all_masks, 'diagnosis_group'] = 'DiagnosisOther'
    
    diagnosis_groups = ['DiagnosisRespiratory','DiagnosisCardiovascular','DiagnosisNeurological',
                        'DiagnosisInfection','DiagnosisGastrointestinal','DiagnosisMetabolic',
                        'DiagnosisTrauma','DiagnosisOther']
    for group in diagnosis_groups:
        df[group] = 0

    # Update the columns based on the presence of each condition in 'preexisting_conditions'
    for group in diagnosis_groups:
        df[group] = df['diagnosis_group'].apply(
            lambda x: 1 if pd.notna(x) and group in x else 0
        )
    ...
    final_column_order =   ['cats_id',
                            ...
                            'day_night',
                            'DiagnosisRespiratory',
                            'DiagnosisCardiovascular',
