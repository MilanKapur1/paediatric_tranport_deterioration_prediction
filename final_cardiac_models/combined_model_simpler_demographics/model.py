...
class DynamicModel(nn.Module):
    ...
    def forward(self, dynamic_features, missingness_mask=None):
        ...
        # Take the last token (assumes input shape [batch, seq_len, d_model])
...
def load_model(model_path,
            ...
            ff_dropout=0,
            batch_norm=False,
            scalar_input_dim=140,
            scalar_mlp_hidden_dim=256,
            embedding_hidden_dim=256,
            ensemble_mlp_hidden_dim=512,
            output_dim=1,
            device="cpu",
            freeze=True):  # New parameter to control freezing
    """
    Load and optionally freeze the model.
    
    Args:
        model_path (str): Path to the saved model weights.
        freeze (bool): If True, the model will be frozen (no training).
    
    Returns:
        model: Loaded model (frozen if specified).
    """
    ...
    model = EnsembleModel(dynamic_model, static_model, ensemble_mlp_hidden_dim, 
                          output_dim, batch_norm, scalar_mlp_hidden_dim, 
                          embedding_hidden_dim, dynamic_dim_out)
...
def run_exp(experiment_name,
    ...
    # Define ensemble model
    model = EnsembleModel( dynamic_model, static_model, ensemble_mlp_hidden_dim, output_dim,batch_norm,scalar_mlp_hidden_dim,embedding_hidden_dim,dynamic_dim_out)
    model = torch.compile(model)
    model.to(device)
...
def load_model_for_eval():
    ...
    subfolder = "final_cardiac_models/combined_model_simpler_demographics"
