...
module_path = '/home/workspace/files/MilanK/Model1/final_models/final_cardiac_models/combined_model_simpler_demographics'
# Add the module's directory to the system path if it's not already present
if module_path not in sys.path:
    sys.path.append(module_path)
...
class PatientDataset(Dataset):
    ...
    def __init__(self, patient_list, is_train = False, min_window_min=15, step_min=1, max_window_min=60, prediction_window_length=15, oversample_label=None):
        ...
        # To ensure label window accurately represents full 15-minute window,
        # stop sliding windows too close to the end so that a full prediction window is available.
        self.samples_15min = []
        for cats_id, file_name in tqdm(self.files.items(), desc="Generating 15-min samples"):
            ...
            self.samples_15min.extend(patient_samples)

        self.static_data = load_static_data(self.patient_list)
        self.static_data.set_index('cats_id', inplace=True)

        print (f"Number of windows without minority oversampling: {len(self.samples_15min)}" )
        
        self.samples = self.samples_15min
        if self.is_train:
            self._generate_final_samples()
            self.samples = list(set(self.samples)) ##remove duplicate tuples
            
            print (f"Number of windows after minority oversampling: {len(self.samples)}" )
    ...
    def _get_label_for_sample(self, cats_id, start_min, end_min):
        if cats_id not in self.cache:
            file_path = self.files[cats_id]
            data, resp_df, cardiac_df = load_data(file_path)
        ...
        if resp_df.empty:
            ...
            resp_minutes_list = (pd.to_timedelta(resp_df["devTime"]).dt.total_seconds() / 60).astype(int).tolist()
        
        if cardiac_df.empty:

            cardiac_minutes_list = []
        else:
            cardiac_minutes_list = (pd.to_timedelta(cardiac_df["devTime"]).dt.total_seconds() / 60).astype(int).tolist()

        return  resp_minutes_list, cardiac_minutes_list
    



    def _generate_final_samples(self):
        for cats_id in tqdm(self.files, desc="Generating final samples", unit="patient"):
            start_min = 0
            end_min = 15
    
            resp_minutes_list, cardiac_minutes_list = self._get_label_for_sample(cats_id, start_min, end_min)
    
            if self.oversample_label == 'resp':
                minutes_list = resp_minutes_list
            elif self.oversample_label == 'cardiac':
                minutes_list = cardiac_minutes_list
    
            if not minutes_list:
                continue  # Skip if no valid minutes found
    
            end_min_list = set()
            for item in minutes_list:
                # Use a negative step to create a descending list
                end_min_range = list(range(item, item - 16, -1))
                end_min_list.update(end_min_range)
    
            end_min_list = sorted(end_min_list, reverse=True)
    
            # Convert end_min_list to a NumPy array
            end_min_arr = np.array(end_min_list)
            end_min_arr = end_min_arr[end_min_arr >= 15]
            
            
            # Create an array for cats_id with the same shape as end_min_arr
            cats_arr = np.full(end_min_arr.shape, cats_id)
    
            # Compute the second column (item - 15) and keep the third column as is
            sample_arr = np.column_stack((cats_arr, end_min_arr - 15, end_min_arr))
    
            # Convert to list of tuples and extend self.samples
            sample_tuples = [tuple(row) for row in sample_arr]
            self.samples.extend(sample_tuples)
...
#####################################################################################
        ...
        # Handle NaN values in features by imputing to 0 - this is neutral and better than -1 as 
